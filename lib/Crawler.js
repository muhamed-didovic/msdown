    ...
    async scrapeCourses(opts, url) {
        ...
        return await this.withBrowser(async (browser) => {
            return await this.withPage(browser)(async (page) => {
                ...
                await fs.writeFile(`./json/courses-${new Date().toISOString()}.json`, JSON.stringify(courses, null, 2), 'utf8')
                            ...
                            .then(async items => {
                                ...
                                await Promise.all([
                                    (async () => {
                                        ...
                                        await fs.ensureDir(path.resolve(process.cwd(), 'json'))
                                        await fs.writeFile(`./json/test-${new Date().toISOString()}.json`, JSON.stringify(items, null, 2), 'utf8')
                                    })(),
                                    (async () => {
                                        //download videos
